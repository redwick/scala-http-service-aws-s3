...
object DBManager {
  ...
  trait DBAccessTable[A] extends Table[A]{
    def removed = column[Int]("removed")
    def user_created = column[String]("user_created")
    def user_updated = column[String]("user_updated")
    def date_created = column[Long]("date_created")
    def date_updated = column[Long]("date_updated")
  }
